...
describe("KandelStatus unit tests suite", () => {
  ...
  function assertEqual(
    ...
    expected: { live: boolean; offerId: number; price: Big },
    i: number
  ) {
    assert.equal(
      actual === undefined,
  ...
  describe("getOfferStatuses", () => {
    it("throws on no live offers", () => {
      assert.throws(
        () =>
          sut.getOfferStatuses(Big(5), Big(1), 10, [
    ...
    it("gets all as expected for initial distribution", () => {
      ...
      const dist = sut.calculation.calculateDistribution(
        Big(2),
        Big(2000),
        ratio,
        pricePoints
      ...
      const prices = sut.calculation.getPrices(dist);
    ...
    it("gets price and status with dead and crossed offers", () => {
      ...
      // Act
      const statuses = sut.getOfferStatuses(midPrice, ratio, pricePoints, [
