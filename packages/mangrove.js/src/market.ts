...
class Market {
  ...
  /* Await until mangrove.js has precessed an event that matches `filter`
     as part of the transaction generated by `tx`. The goal is to reuse the
     event processing facilities of market.ts as much as possible but still
     be tx-specific (and in particular fail if the tx fails).
     Alternatively one could just use `await (await tx).wait(1)` but then
     you would not get the context provided by market.ts (current position of a new offer in the OB, for instance).
  */
  ...
  onceWithTxPromise<T>(
    ...
    return new Promise((ok, ko) => {
      ...
      txPromise.then((resp) => {
        // Warning: if the tx does not revert but the `filter` function never succeeds, the `once` callback will never be triggered and memory leak by queuing tasks.
