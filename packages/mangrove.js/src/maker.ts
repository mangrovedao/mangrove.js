...
export class SimpleMaker {
  ...
  getOutboundInbound(ba: "bids" | "asks"): {
    ...
    let [outbound_tkn, inbound_tkn] = [this.market.base, this.market.quote];
    if (ba === "bids") {
      // swap in/out if bidding
      [outbound_tkn, inbound_tkn] = [inbound_tkn, outbound_tkn];
    }

    return { outbound_tkn, inbound_tkn };
  }
  ...
  */
  newOffer(
    ...
    const { outbound_tkn, inbound_tkn } = this.getOutboundInbound(p.ba);
    ...
    this.contract
      .newOffer(
        ...
        ethers.BigNumber.from(2).pow(256).sub(1), // gasprice
        0,
        this.market.getPivot(p.ba, price)
      )
      .then((_resp) => (resp = _resp));

    return new Promise((ok) => {
      this.market.once(
        (cbArg: bookSubscriptionCbArgument, evt: any, _evt: ethers.Event) =>
          ok({ id: cbArg.offer.id, event: _evt }),
        (cbArg: any, evt: any, _evt: ethers.Event) => {
          if (typeof resp !== "undefined") {
            return resp.hash === _evt.transactionHash;
          } else {
            return false;
          }
        }
  ...
  cancelOffer(
    ...
    let resp: TransactionResponse;
    this.contract
      .retractOffer(outbound_tkn.address, inbound_tkn.address, id, deprovision)
      .then((_resp) => (resp = _resp));

    return new Promise(async (ok) => {
      this.market.once(
        (cbArg: bookSubscriptionCbArgument) => ok(),
        (cbArg: any, evt: any, _evt: ethers.Event) => {
          if (typeof resp !== "undefined") {
            return resp.hash === _evt.transactionHash;
          } else {
