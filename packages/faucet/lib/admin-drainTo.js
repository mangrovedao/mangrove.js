// This script drains the specified amounts of each of the below tokens from their faucet contracts to the specified address.
const ethers = require("ethers");

const receiver = "0x3073A02460D7BE1A1C9afC60A059Ad8d788A4502";
...
const tokenDrainTo = async ({ symbol, amount, decimals, address, faucet }) => {
  const contract = new ethers.Contract(
    faucet,
    ["function drainTo(address dest, uint amt)"],
    signer
  );
  await contract
    .drainTo(receiver, ethers.utils.parseUnits(amount, decimals))
    .then((tx) => tx.wait());
};

for (const token of tokens) {
  await tokenDrainTo(token);
}
