...
async function logLenderStatus(
  contract,
  lenderName,
  tokens,
  account,
  ...
  console.log(
    `**** ${lenderName} borrow power (${baseName}): \x1b[35m`,
    formatToken(borrowPower, baseUnit),
    "\x1b[0m ****"
  );
  for (const symbol of tokens) {
    switch (lenderName) {
      case "compound": {
        ...
        const token = await getContract(symbol);
        const [redeemable] = await contract.callStatic.maxGettableUnderlying(
          cToken.address,
          account
        );
        const balance = await token.balanceOf(treasury ? treasury : account);
...
async function marketOrder(
  ...
  const receipt = await moTx.wait();

  console.log(
    "\t",
    chalk.bgGreen.black(
      `ORDER [${formatToken(wants, await getDecimals(base_sym))}/${formatToken(
        gives,
        await getDecimals(quote_sym)
      )}] FULFILLED`
    ),
    "[",
    chalk.green(formatToken(takerGot, await getDecimals(base_sym)) + base_sym),
    " | ",
    chalk.red(formatToken(takerGave, await getDecimals(quote_sym)) + quote_sym),
    " | ",
    chalk.yellow(formatToken(bounty, 18) + "ETH"),
    "| ",
...
async function snipeFail(
  ...
  const [successes, , , bounty] = await mgv.callStatic.snipes(
    base.address,
    quote.address,
    [
      [
  ...
  assert(successes.eq(0), "Snipe did not fail");
  ...
  console.log(
    "\t",
    chalk.bgRed.black("OFFER FAILED"),
    "[",
    chalk.green(formatToken(wants, await getDecimals(base_sym)) + base_sym),
    " | ",
    chalk.red(formatToken(gives, await getDecimals(quote_sym)) + quote_sym),
...
function listenMgv(mgv) {
  ...
  mgv.once(filter_posthook, (out_tkn, in_tkn, offerId, data, event) => {
    console.log(
      chalk.red(
        `Posthook Reverted [Offer ${offerId}] reason: ${ethers.utils.parseBytes32String(
          data
        )}`
      )
    );
...
function listenOfferLogic(once, logic, reason = null) {
  const filter_fail = logic.filters.LogIncident();
  if (once) {
    logic.once(filter_fail, (mgv, outTkn, inTkn, offerId, data) => {
      let reason_ = ethers.utils.parseBytes32String(data);
      if (reason) {
        assert(
          reason === reason_,
          `Incorrect log in offerProxy, expected: ${reason}, received:${reason_}`
        );
      }
      console.log(
        chalk.green(`\t\u2713`),
        `LogIncident: offer ${offerId.toString()}`,
        chalk.yellow(`${ethers.utils.parseBytes32String(data)}`)
      );
    });
  } else {
