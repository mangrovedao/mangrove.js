...
contract MakerPosthookTest is MangroveTest, IMaker {
  ...
  function renew_offer_at_posthook(MgvLib.SingleOrder calldata order, MgvLib.OrderResult calldata) internal {
    called = true;
    mgv.updateOffer(
      order.outbound_tkn, order.inbound_tkn, 1 ether, 1 ether, gasreq, _gasprice, order.offerId, order.offerId
    );
  }
  ...
  function check_best_in_posthook(MgvLib.SingleOrder calldata order, MgvLib.OrderResult calldata) internal {
    called = true;
    (, Structs.LocalPacked cfg) = mgv.config(order.outbound_tkn, order.inbound_tkn);
  ...
  function test_best_in_posthook_is_correct() public {
    ...
    ofr = mgv.newOffer($(base), $(quote), 1 ether, 1 ether, gasreq, _gasprice, 0);
    uint best = mgv.newOffer($(base), $(quote), 0.5 ether, 1 ether, gasreq, _gasprice, 0);
