...
contract OfferForwarderTest is OfferLogicTest {
  ...
  function test_derivedGaspriceIsAccurateEnough(uint fund) public {
    ...
    assertEq(mgv.balanceOf(address(makerContract)), contractOldBalance + loss_for_maker, "Invalid contract balance");
    ...
    assertTrue(loss_for_maker < (15000 * global.gasprice() * 10 ** 9), "rounding exceeds storage write cost");
  }
  ...
  function test_failedOfferCreditsOwner(uint fund) public {
    ...
    assertTrue(approx_cost < 1_000 * global.gasprice() * 10 ** 9, "Approximation of offer owner's credit is too coarse");
  }
}
