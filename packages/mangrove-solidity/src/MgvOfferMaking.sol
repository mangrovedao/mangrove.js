...
import {IMaker, HasMgvEvents, Structs} from "./MgvLib.sol";
...
contract MgvOfferMaking is MgvHasOffers {
  ...
  struct OfferPack {
    ...
    uint gasprice;
    uint pivotId;
    Structs.GlobalPacked global;
    Structs.LocalPacked local;
    // used on update only
    Structs.OfferPacked oldOffer;
  }
  ...
  */
  function updateOffer(
    ...
    unchecked {
      ...
      // Save local config
      Structs.LocalPacked oldLocal = ofp.local;
  ...
  {
    unchecked {
      (, Structs.LocalPacked local) = config(outbound_tkn, inbound_tkn);
      unlockedMarketOnly(local);
      Structs.OfferPacked offer = offers[outbound_tkn][inbound_tkn][offerId];
      Structs.OfferDetailPacked offerDetail = offerDetails[outbound_tkn][inbound_tkn][offerId];
      ...
      if (isLive(offer)) {
        Structs.LocalPacked oldLocal = local;
  ...
  */
  ...
  function fund(address maker) public payable {
    unchecked {
      (Structs.GlobalPacked _global,) = config(address(0), address(0));
      liveMgvOnly(_global);
  ...
  function writeOffer(OfferPack memory ofp, bool update) internal {
    ...
    **Warning**: `findPosition` will call `better`, which may read the offer's `offerDetails`. So it is important to find the offer position _before_ we update its `offerDetail` in storage. We waste 1 (hot) read in that case but we deem that the code would get too ugly if we passed the old `offerDetail` as argument to `findPosition` and to `better`, just to save 1 hot read in that specific case.  */
      ...
      uint oldProvision;
      {
        Structs.OfferDetailPacked offerDetail = offerDetails[ofp.outbound_tkn][ofp.inbound_tkn][ofp.id];
        if (update) {
        ...
        if (
          ...
          offerDetails[ofp.outbound_tkn][ofp.inbound_tkn][ofp.id] = Structs.OfferDetail.pack({
            __maker: msg.sender,
            __gasreq: ofp.gasreq,
    ...
    As a note for future changes, there is a tricky edge case where `prev == oldPrev` yet the prev/next should be changed: a previously-used offer being brought back in the book, and ending with the same prev it had when it was in the book. In that case, the neighbor is currently pointing to _another_ offer, and thus must be updated. With the current code structure, this is taken care of as a side-effect of checking `!isLive`, but should be kept in mind. The same goes in the `next == oldNext` case. */
      ...
      Structs.OfferPacked ofr = Structs.Offer.pack({__prev: prev, __next: next, __wants: ofp.wants, __gives: ofp.gives});
  ...
  function findPosition(OfferPack memory ofp) internal view returns (uint, uint) {
    unchecked {
      ...
      Structs.OfferPacked pivot = pivotId == ofp.id ? ofp.oldOffer : offers[ofp.outbound_tkn][ofp.inbound_tkn][pivotId];
  ...
  function better(OfferPack memory ofp, Structs.OfferPacked offer1, uint offerId1) internal view returns (bool) {
    unchecked {
      if (offerId1 == 0) {
